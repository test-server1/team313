  ...
  class i {
    ...
    createWorld() {
      ;((this.height = this.DOM.element.offsetHeight),
        ...
        this.DOM.element.removeEventListener(
          'mousewheel',
    ...
    createBoundries() {
      ;((this.boundStart = Matter.Bodies.rectangle(
        -250,
        this.height / 2,
        500,
        4 * this.height,
        ...
        (this.boundEnd = Matter.Bodies.rectangle(
          this.width + 250,
          this.height / 2,
          500,
          4 * this.height,
    ...
    createBodies() {
      this.DOM.throwables.forEach((t, e) => {
        const i = t.querySelector('span , div , img'),
          ...
          h = gsap.utils.random(0.2 * -Math.PI, 0.2 * Math.PI),
        ...
        ;(this.bodies.push(u),
          Matter.Composite.add(this.engine.world, [u]),
          Matter.Events.on(this.runner, 'tick', () => {
            this.runner.enabled &&
              ((i.style.transform =
                'translate(-50%, -50%) rotate(' + u.angle.toFixed(2) + 'rad)'),
              o(u.position.y.toFixed(1)),
              n(u.position.x.toFixed(1)))
          }))
      })
    }
    createTopBound() {
    ...
    makeScrollGravity() {
      ...
      Matter.Events.on(this.runner, 'tick', () => {
        const e =
          ...
          i = e - t
        ;((this.engine.gravity.y = 0.7 - gsap.utils.clamp(-2, 4, 0.1 * i)),
          (t = e))
      })
    }
    updateBoundries() {
      ;(this.boundTop &&
        Matter.Body.setVertices(
          this.boundTop,
          Matter.Bodies.rectangle(0, -250, 2 * this.width, 500, {
            isStatic: !0,
          }).vertices,
        ),
        this.boundStart &&
          (Matter.Body.setPosition(this.boundStart, {
            x: -250,
            y: this.height / 2,
          }),
          Matter.Body.setVertices(
        ...
        this.boundEnd &&
          (Matter.Body.setPosition(this.boundEnd, {
            x: this.width + 250,
            y: this.height / 2,
          }),
          Matter.Body.setVertices(
        ...
        this.boundBottom &&
          ...
          Matter.Body.setVertices(
            this.boundBottom,
            Matter.Bodies.rectangle(0, this.height + 250, 2 * this.width, 500, {
              isStatic: !0,
            }).vertices,
          )))
    }
    updateBodies() {
      this.DOM.throwables.forEach((t, e) => {
        ...
        if (
          (Matter.Body.setVertices(i, o.vertices),
          i.position.y > this.height &&
    ...
    startRain() {
      ...
      Matter.Events.on(this.runner, 'tick', () => {
        !t &&
          this.bodies[this.bodies.length - 1].position.y >
            this.DOM.element.offsetHeight / 2 &&
          (this.createTopBound(),
          this.options.scrollGravity && this.makeScrollGravity(),
          (t = !0))
      })
    }
    refresh() {
